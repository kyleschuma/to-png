const cloneElement = el => {
  ...
  const cloneChildren = () => Promise.all([...el.childNodes].map(child => cloneElement(child)));
  const copyCSS = () =>
    new Promise(accept => {
      setTimeout(() => {
        ...
        if (el.nodeType === Node.ELEMENT_NODE) {
          ...
          const styles = [...source];
          styles.forEach(name => target.setProperty(name, source.getPropertyValue(name)));
        }
        accept();
      }, 10);
    });
...
export default (...args) =>
  supportsForeignObjects() ? toCanvas(...args).then(toResponse) : toWorkAround(...args);
